...
public partial class V1Client : IV1Client
{
    ...
    private async global::System.Threading.Tasks.Task<
        WithRawResponse<CountsResponse>
    > GetCountsAsyncCore(
    ...
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<CountsResponse>(responseBody)!;
                return new WithRawResponse<CountsResponse>()
                {
                    Data = responseData,
                    RawResponse = new global::Candid.Net.RawResponse()
                    {
    ...
    /// </code></example>
    public WithRawResponseTask<CountsResponse> GetCountsAsync(
        CountsRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<CountsResponse>(
            GetCountsAsyncCore(request, options, cancellationToken)
        );
    }
